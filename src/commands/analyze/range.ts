...
static override flags = {
    ...
    from: Flags.string({char: 'f', description: 'Start date (YYYY-MM-DD format)', required: true}),
    ...
    to: Flags.string({char: 't', description: 'End date (YYYY-MM-DD format)', required: true}),
  }
  ...
  async run(): Promise<void> {
    ...
    if (!fromDate || !toDate) this.error('Invalid date format. Use YYYY-MM-DD format.')
    if (fromDate > toDate) this.error('Start date must be before end date.')
    ...
    try {
      ...
      const result = await pipeline.analyzeDateRange(fromDate!, toDate!, options)
      if (flags.json) {
